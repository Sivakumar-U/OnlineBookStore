...
import com.bridgelabz.onlinebookstore.exception.EmailSendingException;
...
public class MailServiceUtility {
	...
	private MimeMessage mimeMessageConfiguration(Session session, String toEmail, String subject, String body) {
		...
		try {
			...
			mimeMessage.setFrom(new InternetAddress(SENDER_EMAIL_ID, "OnlineBookStore"));
	...
	public boolean recievedMessage(EmailObject mailObject) {

		if (sendMail(mailObject.getEmail(), mailObject.getSubject(), mailObject.getMessage())) { 
			return true;
		}
		else throw new EmailSendingException("Error in Sending mail!", HttpStatus.BAD_GATEWAY.value());
